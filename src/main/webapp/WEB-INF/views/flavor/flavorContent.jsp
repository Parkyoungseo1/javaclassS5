...
<head>
  ...
  <script>
    ...
    function flavorDelete() {
    	let ans = confirm("현재 게시글을 삭제 하시겠습니까?");
    	if(ans) location.href = "flavorDelete?idx=${vo.idx}";
    }
    ...
    function replyCheck() {
    	...
    	$.ajax({
    		url  : "${ctp}/flavor/flavorReplyInput",
    		type : "post",
    		data : query,
    		success:function(res) {
    			if(res != "0") {
    ...
    function replyCheckRe(idx, re_step, re_order) {
    	...
    	$.ajax({
    		url  : "${ctp}/flavor/flavorReplyInputRe",
    		type : "post",
    		data : query,
    		success:function(res) {
    			if(res != "0") {
...
<div class="container">
  ...
  <div class="d-flex justify-content-between mt-4">
    <div>
      <c:if test="${empty flag}"><button onclick="location.href='flavorList?pag=${pag}&pageSize=${pageSize}';" class="btn btn-warning">돌아가기</button></c:if>
      <c:if test="${!empty flag}"><button onclick="location.href='flavorSearch?pag=${pag}&pageSize=${pageSize}&search=${search}&searchString=${searchString}';" class="btn btn-warning">돌아가기</button></c:if>
    </div>
    <div>
      <c:if test="${sNickName == vo.nickName || sLevel == 0}">
        <c:if test="${report == 'OK'}"><span class="text-danger font-weight-bold mr-2">현재 이글은 신고중입니다.</span></c:if>
        <button onclick="location.href='flavorUpdate?idx=${vo.idx}&pag=${pag}&pageSize=${pageSize}';" class="btn btn-primary mr-2">수정</button>
        <button onclick="flavorDelete()" class="btn btn-danger">삭제</button>
      </c:if>
